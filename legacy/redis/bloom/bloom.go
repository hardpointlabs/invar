...
func bloomPageSuffix(name []byte, filterID, pageNum uint64) []byte {
	s := string(name) + "\x00bf:" + strconv.FormatUint(filterID, 10) + ":p:" + strconv.FormatUint(pageNum, 10)
	return []byte(s)
}

func bloomReadMeta(tx kv.Tx, session *common.Session, key []byte) (*bloomMeta, error) {
...
func bloomReadMetaOrNil(tx kv.Tx, session *common.Session, key []byte) (*bloomMeta, error) {
	m, err := bloomReadMeta(tx, session, key)
...
func bloomWriteMeta(tx kv.Tx, session *common.Session, key []byte, m *bloomMeta) error {
	data, err := encodeBloomMeta(m)
	if err != nil {
		return err
	}
	return tx.Set(session.NewPublicEntry(key, data).Metadata(byte(common.RedisBloom)))
}

func bloomReadPage(tx kv.Tx, session *common.Session, name []byte, filterID, pageNum uint64) ([]byte, error) {
	key := bloomPageSuffix(name, filterID, pageNum)
	item, err := tx.Get(session.PrivateKey(key))
	if err == kv.ErrKeyNotFound {
...
func bloomWritePage(tx kv.Tx, session *common.Session, name []byte, filterID, pageNum uint64, data []byte) error {
	key := bloomPageSuffix(name, filterID, pageNum)
	return tx.Set(session.NewPrivateEntry(key, data).Metadata(byte(common.RedisBloom)))
}

// --- Core add logic (shared by Bfadd, Bfmadd, Bfinsert) ---

func bloomAddOne(tx kv.Tx, session *common.Session, key, item []byte) (int, error) {
	meta, err := bloomReadMetaOrNil(tx, session, key)
	if err != nil {
		return 0, err
	}
	if meta == nil {
		...
		if err := bloomWriteMeta(tx, session, key, meta); err != nil {
			return 0, err
		}
	}
	...
	for _, pos := range positions {
		...
		if _, ok := pm[pageNum]; !ok {
			pdata, err := bloomReadPage(tx, session, key, filter.ID, pageNum)
			if err != nil {
				return 0, err
			}
	...
	for _, pr := range pm {
		if pr.dirty {
			if err := bloomWritePage(tx, session, key, filter.ID, pr.num, pr.data); err != nil {
				return 0, err
			}
		}
	}
...
func bloomExistsOne(tx kv.Tx, session *common.Session, key, item []byte) (bool, error) {
	meta, err := bloomReadMetaOrNil(tx, session, key)
	if err != nil {
		return false, err
	}
	...
	for i := len(meta.Filters) - 1; i >= 0; i-- {
		...
		for _, pos := range positions {
			pageNum := pos / bloomPageBits
			pageData, err := bloomReadPage(tx, session, key, f.ID, pageNum)
			if err != nil {
				return false, err
			}
...
func Bfreserve(session *common.Session, key []byte, errorRate float64, capacity uint64, expansion int, nonScaling bool) common.QueuedOp {
	dbOp := func(tx kv.Tx) (any, error) {
		...
		return nil, bloomWriteMeta(tx, session, key, meta)
	}
...
func Bfadd(session *common.Session, key, item []byte) common.QueuedOp {
	dbOp := func(tx kv.Tx) (any, error) {
		return bloomAddOne(tx, session, key, item)
	}
...
func Bfexists(session *common.Session, key, item []byte) common.QueuedOp {
	dbOp := func(tx kv.Tx) (any, error) {
		return bloomExistsOne(tx, session, key, item)
	}
...
func Bfmadd(session *common.Session, key []byte, items [][]byte) common.QueuedOp {
	dbOp := func(tx kv.Tx) (any, error) {
		results := make([]int, len(items))
		for i, item := range items {
			r, err := bloomAddOne(tx, session, key, item)
			if err != nil {
				return nil, err
			}
			results[i] = r
...
func Bfmexists(session *common.Session, key []byte, items [][]byte) common.QueuedOp {
	dbOp := func(tx kv.Tx) (any, error) {
		results := make([]int, len(items))
		for i, item := range items {
			exists, err := bloomExistsOne(tx, session, key, item)
			if err != nil {
				return nil, err
			}
			if exists {
...
func Bfinsert(session *common.Session, key []byte, info *InsertInfo) common.QueuedOp {
	dbOp := func(tx kv.Tx) (any, error) {
		meta, err := bloomReadMetaOrNil(tx, session, key)
		if err != nil {
			return nil, err
		...
		results := make([]int, len(info.Items))
...
func Bfinfo(session *common.Session, key []byte) common.QueuedOp {
	dbOp := func(tx kv.Tx) (any, error) {
		meta, err := bloomReadMetaOrNil(tx, session, key)
		if err != nil {
			return nil, err
